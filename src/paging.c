...
void init_paging() {
    printf("Given that a page size is %d bytes and there are %d free bytes, there are %d free pages\n", PAGE_SIZE, NUM_FREE_BYTES, NUM_PAGES);
    printf("The calculation for NUM_FREE_BYTES is %d-%d=%d\n", MMIO_BASE, START_OF_KERNEL_STACK_CPU2, NUM_FREE_BYTES);
    printf("Paging Start Address: %d\n", &__paging_start);
...
void *alloc_page() {
    printf("Alloc page called....");
    for (int i = 0; i < NUM_PAGES; ++i) {
        if (paging_map_arr[i].isAllocated) {
            continue;
        }
        printf("giving page %d to the caller with page address %p\n", i, (&__paging_start + i*PAGE_SIZE));
...
void free_page(void *page_adr) {
    ...
    printf("Free page called with page address %p, this seems to have been from index %d\n", page_adr, idx);
